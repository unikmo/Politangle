...
type TeacherSummary = {
  ...
  roomLabel: string;
  joinedCount: number;
  activity: { type: string; title: string; questionIds: string[]; pacing: 'teacher' | 'student'; projectorMode: 'live' | 'reveal'; lessonId?: string };
  currentIndex: number;
...
export default function TeacherSchoolClient() {
  ...
  const [activityType, setActivityType] = useState<'quick26' | 'full42' | 'literacy' | 'guided' | 'custom'>('quick26');
  ...
  function activityPayload() {
    if (activityType === 'guided') return { type: activityType, lessonId, pacing, projectorMode };
    if (activityType === 'custom') return { type: activityType, title: customIds.length === 1 ? 'Single live question' : 'Custom activity', questionIds: customIds, pacing, projectorMode };
    return { type: activityType, pacing, projectorMode };
  }
  ...
  const configuration = (
    <article className="engine-card school-config-card">
      ...
      <div className="school-config-grid">
        ...
        <label><span>Activity</span><select value={activityType} onChange={(e) => setActivityType(e.target.value as typeof activityType)}><option value="quick26">Quick 26</option><option value="full42">Full 42</option><option value="literacy">Political Literacy Quiz</option><option value="guided">Guided Lesson</option><option value="custom">Build my own / single question</option></select></label>
        {activityType === 'guided' && <label><span>Lesson</span><select value={lessonId} onChange={(e) => setLessonId(e.target.value)}>{schoolLessons.map((item) => <option key={item.id} value={item.id}>{item.title} · {item.duration}</option>)}</select></label>}
      ...
      {activityType === 'custom' && <details className="engine-details" open><summary>Select approved questions ({customIds.length} selected)</summary><div className="school-question-picker">{activityOptions.full42.map((id) => <label key={id}><input type="checkbox" checked={customIds.includes(id)} onChange={() => toggleCustom(id)} /> {classroomQuestionLabel(id)}</label>)}{activityOptions.literacy.map((id) => <label key={id}><input type="checkbox" checked={customIds.includes(id)} onChange={() => toggleCustom(id)} /> {classroomQuestionLabel(id)}</label>)}</div></details>}
  ...
  return (
    <section className="engine-shell school-shell school-teacher-print">
      <div className="school-room-strip"><strong>{summary.roomLabel || `Class ${summary.code}`}</strong><span>{summary.activity.title}</span><span>{summary.joinedCount} joined</span><span>{summary.status}</span></div>
