...
type ClassroomQuestion = LocalizableSchoolQuestion & { title: string; construct?: string; mode?: string };
type Distribution = { id: string; kind: string; title: string; responses: number; distribution: { id: string; label: string; count: number; percent: number }[]; correct: number | null; correctPercent: number | null };
type PublicRoom = {
  code: string; status: 'active' | 'closed'; active: boolean; roomLabel: string; joinedCount: number;
  ...
  currentIndex: number; currentQuestionId: string | null; questionOpen: boolean; revealed: boolean;
  currentQuestion: ClassroomQuestion | null; activityQuestions: ClassroomQuestion[]; projectorDistribution: Distribution | null;
};
...
function DistributionChart({ data, optionLabel }: { data: Distribution; optionLabel: (id: string, fallback: string) => string }) {
  return <div className="school-distribution">{data.distribution.map((row) => <div className="school-bar-row" key={row.id}><div className="school-bar-label"><span>{optionLabel(row.id, row.label)}</span><strong>{row.percent}% · {row.count}</strong></div><div className="school-bar-track"><span style={{ width: `${row.percent}%` }} /></div></div>)}</div>;
}
...
export default function StudentSchoolClient({ initialCode }: { initialCode: string }) {
  ...
  useEffect(() => { const stored = sessionStorage.getItem(CODE_KEY) ?? ''; if (!initialCode && stored) setEntryCode(stored); }, [initialCode]);
  useEffect(() => {
    if (!code || !token) return;
    let cancelled = false;
    const refresh = async () => { const response = await fetch(`/api/school/classes/${encodeURIComponent(code)}`, { cache: 'no-store' }).catch(() => null); if (!cancelled && response?.ok) setRoom(await response.json() as PublicRoom); };
    void refresh(); const id = window.setInterval(refresh, 1200); return () => { cancelled = true; window.clearInterval(id); };
  }, [code, token]);

  async function join() {
    const normalized = entryCode.trim().toUpperCase();
    if (normalized.length !== 6) { setMessage(copy.codeError); return; }
    setBusy(true); setMessage(copy.joining);
    ...
    if (!nextToken) { nextToken = crypto.randomUUID(); sessionStorage.setItem(tokenKey(normalized), nextToken); }
    const response = await fetch(`/api/school/classes/${encodeURIComponent(normalized)}/join`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ participantToken: nextToken }) }).catch(() => null);
    ...
    setLocalAnswers(saved.answers ?? {}); setSubmittedIds(saved.submitted ?? []); sessionStorage.setItem(CODE_KEY, normalized); setCode(normalized); setToken(nextToken); setRoom(preview); setMessage(copy.joined); setBusy(false);
  }

  function saveLocal(nextAnswers: Record<string, string | string[]>, nextSubmitted = submittedIds) { setLocalAnswers(nextAnswers); setSubmittedIds(nextSubmitted); if (code) sessionStorage.setItem(answerKey(code), JSON.stringify({ answers: nextAnswers, submitted: nextSubmitted })); }
  const question = useMemo(() => { if (!room) return null; if (room.activity.pacing === 'teacher') return room.currentQuestion; return room.activityQuestions[studentIndex] ?? null; }, [room, studentIndex]);
  ...
  const isJunior = ageBand === 'junior-10-13';
  ...
  function choose(optionId: string) {
    ...
    else if (question.multiSelect) { const existing = Array.isArray(selected) ? selected : []; answer = existing.includes(optionId) ? existing.filter((id) => id !== optionId) : [...existing, optionId]; }
  ...
  async function submit() {
    ...
    setBusy(true);
    const response = await fetch(`/api/school/classes/${encodeURIComponent(room.code)}/submit`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ participantToken: token, questionId: question.id, answer: selected }) }).catch(() => null);
    if (!response?.ok) { setMessage(copy.submitError); setBusy(false); return; }
    const nextSubmitted = [...new Set([...submittedIds, question.id])]; saveLocal(localAnswers, nextSubmitted); setMessage(`${copy.received} ${copy.receivedBody}`); setBusy(false);
  }

  if (!code || !token) return <section className="engine-shell school-shell school-student-shell"><article className="engine-card school-join-card student-simple-card"><p className="engine-kicker">{copy.joinKicker}</p><h1>{copy.joinTitle}</h1><p className="student-privacy-note">{copy.joinHelp}</p><input className="school-code-input" value={entryCode} maxLength={6} onChange={(event) => setEntryCode(event.target.value.toUpperCase().replace(/[^A-Z0-9]/g, ''))} placeholder="ABC234" aria-label={copy.codeLabel} /><div className="engine-result-actions"><button className="engine-primary-link" type="button" disabled={busy} onClick={join}>{busy ? copy.joining : copy.join}</button></div>{message && <p className="school-status-message">{message}</p>}</article></section>;
  ...
  const optionLabel = (id: string, fallback: string) => { if (!question) return fallback; if (question.kind === 'believe') return scale[id] ?? fallback; return localizedSchoolLiteracyOption(locale, question, id, ageBand); };

  return <section className={`engine-shell school-shell school-student-shell ${isJunior ? 'school-junior-student' : 'school-youth-student'}`}>
    ...
    {room.status === 'closed' ? <article className="engine-card student-simple-card"><p className="engine-kicker">{copy.sessionEnded}</p><h1>{copy.closedTitle}</h1><p>{copy.closedBody}</p></article> : studentPacedComplete ? <article className="engine-card student-simple-card"><p className="engine-kicker">{copy.complete}</p><h1>{copy.completeTitle(room.activity.questionIds.length)}</h1><p>{copy.completeBody}</p></article> : question ? <article className="engine-card school-question-card student-question-card">
      ...
      {question.kind === 'believe' ? <><div className="engine-statement student-belief-statement"><p>{localizedSchoolBeliefStatement(locale, question)}</p></div><p className="student-no-correct">{copy.noCorrect}</p></> : <h1 className="student-literacy-question">{localizedSchoolLiteracyPrompt(locale, question, ageBand)}</h1>}
      {!alreadySubmitted && (room.activity.pacing === 'student' || room.questionOpen) ? <><div className={question.kind === 'believe' ? (isJunior ? 'junior-response-options' : 'school-believe-options student-believe-options') : 'deep-options student-literacy-options'}>{question.options.map((option) => { const active = Array.isArray(selected) ? selected.includes(option.id) : selected === option.id; const label = optionLabel(option.id, option.label); return <button key={option.id} type="button" aria-label={label} className={active ? 'deep-option selected' : 'deep-option'} onClick={() => choose(option.id)}>{question.kind === 'believe' && !isJunior ? <><b>{option.id === 'unsure' ? '?' : Number(option.id) > 0 ? `+${option.id}` : option.id.replace('-', '−')}</b><span>{label}</span></> : label}</button>; })}</div><div className="engine-result-actions"><button className="engine-primary-link student-submit" type="button" disabled={busy || selected === undefined || (Array.isArray(selected) && !selected.length)} onClick={submit}>{busy ? copy.submitting : copy.submit}</button></div></> : alreadySubmitted ? <div className="school-submitted"><strong>{copy.received}</strong><span>{copy.waitingClass}</span></div> : <p className="engine-callout">{copy.waitingOpen}</p>}
      {revealForCurrent && room.projectorDistribution && <div className="school-reveal-panel"><p className="engine-kicker">{copy.roomAnswers} · {room.projectorDistribution.responses}</p><DistributionChart data={room.projectorDistribution} optionLabel={optionLabel} />{question.kind === 'literacy' && question.explanation && <div className="deep-explanation"><strong>{copy.explanation}</strong><br />{localizedSchoolLiteracyExplanation(locale, question, ageBand)}</div>}</div>}
    ...
    </article> : <article className="engine-card student-simple-card"><p className="engine-kicker">{copy.lobby}</p><h1>{copy.youreIn}</h1><p>{copy.waitingNext}</p></article>}
  </section>;
}
