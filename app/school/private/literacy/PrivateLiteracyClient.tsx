...
function ui(locale: Locale) {
  ...
  if (locale === 'pt-br') return { result:'Seu resultado privado de conhecimento político', private:'Este resultado fica apenas neste navegador. Ele não é enviado para uma turma nem para o professor.', again:'Fazer novamente', kicker:(n:number,total:number)=>`Conhecimento político · ${n} de ${total}`, previous:'Anterior', next:'Próxima', answered:(n:number,total:number)=>`${n}/${total} respondidas`, see:'Ver resultado', notice:'A interface está em português do Brasil, mas estas 15 perguntas continuam em inglês enquanto uma versão em português revisada é validada.' };
