...
export async function POST(request: Request) {
  ...
  await getAdminDb().collection('certificationUsers').doc(user.uid).set({ certificationAgeConfirmedAt: now, updatedAt: now }, { merge: true });
  return NextResponse.json({ certificationAgeConfirmed: true }, { headers: noStore });
}
