...
export function deepGap(locale: Locale, think: number | null, feel: number | null, act: number | null) {
  ...
  if (pairs[0].distance <= 30 && outsider) {
    ...
    if (locale === 'pt-br') return outsider.key === 'THINK' ? 'Seu princípio geral é o ponto que mais se afasta; intuição e decisão prática ficam mais próximas. Uma regra que você apoia em abstrato pode mudar quando a situação fica concreta.' : outsider.key === 'FEEL' ? 'Sua reação intuitiva é o ponto que mais se afasta; princípio e decisão prática ficam mais próximos. O tema parece gerar uma reação emocional que você não levaria por completo para uma regra ou ação.' : 'Sua decisão prática é o ponto que mais se afasta; princípio e intuição ficam mais próximos. Consequências, viabilidade ou dilemas concretos mudam o que você faria.';
...
export function deepNuance(locale: Locale, item: { id: string; name: string; explanation: string; caution?: string }) {
  const localized: Partial<Record<string, Record<'de'|'es'|'fr'|'pt-br', { name: string; explanation: string; caution?: string }>>> = {
    'democratic-socialist-tendency': {
      ...
      'pt-br':{ name:'Tendência socialista democrática', explanation:'Suas respostas combinam uma preferência clara por mais propriedade social, pública, cooperativa ou dos trabalhadores com regras democráticas pluralistas e limites ao poder estatal concentrado.', caution:'É uma nuance dentro da família socialista ampla, não uma ideologia principal adicional nem um rótulo de identidade.' },
    },
    'far-right-radical-right-pattern': {
      ...
      'pt-br':{ name:'Padrão de triagem: direita radical populista', explanation:'Suas respostas combinam pertencimento nacional mais ligado à origem, preferências mais autoritárias por ordem e uma leitura populista que opõe povo e elite. O Politangle, porém, ainda não mede nativismo diretamente.', caution:'É apenas um padrão de triagem. Não equivale ao conservadorismo comum, ao fascismo nem à “extrema direita” e não deve ser lido como um rótulo pessoal de extremismo.' },
    },
  };
