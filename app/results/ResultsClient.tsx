  ...
  return (
    <section className="engine-shell result-shell">
      <article className="engine-card politangle-home-card">
        ...
        <div className="politangle-family-story" aria-label={locale === 'de' ? 'Nächste politische Traditionen' : locale === 'es' ? 'Tradiciones políticas más cercanas' : locale === 'fr' ? 'Traditions politiques les plus proches' : locale === 'pt-br' ? 'Tradições políticas mais próximas' : 'Closest political traditions'}>
          {topFamilies.map((family, index) => {
            const display = familyDisplay(locale, family.id, family.name);
            const label = locale === 'de' ? (index === 0 ? 'Stärkste Nähe' : index === 1 ? 'Deutliche Nähe' : 'Weiterer Einfluss') : locale === 'es' ? (index === 0 ? 'Coincidencia principal' : index === 1 ? 'Cercanía importante' : 'Otra influencia') : locale === 'fr' ? (index === 0 ? 'Proximité principale' : index === 1 ? 'Proximité importante' : 'Autre influence') : locale === 'pt-br' ? (index === 0 ? 'Maior proximidade' : index === 1 ? 'Proximidade importante' : 'Outra influência') : (index === 0 ? 'Main home' : index === 1 ? 'Significant leaning' : 'Additional influence');
        ...
        {people.length > 0 && home.primary && (() => {
          ...
          return <p className="home-reference"><strong>{locale === 'de' ? `Historische Bezugspunkte für ${display.name}:` : locale === 'es' ? `Referencias históricas de ${display.name}:` : locale === 'fr' ? `Repères historiques pour ${display.name} :` : locale === 'pt-br' ? `Referências históricas de ${display.name}:` : `Historical reference points for ${display.name}:`}</strong> {people.join(' · ')}. {locale === 'de' ? 'Sie stehen für die Tradition – nicht für dein persönliches Profil.' : locale === 'es' ? 'Representan la tradición, no tu perfil personal exacto.' : locale === 'fr' ? 'Ils illustrent la tradition, pas ton profil personnel exact.' : locale === 'pt-br' ? 'Elas ilustram a tradição, não o seu perfil pessoal exato.' : 'They illustrate the tradition, not your exact personal profile.'}</p>;
        })()}

        <PoliticalShape axes={result.polygon} locale={locale} />
        <div className="result-action-row compact-actions">
          <button className="engine-primary-link" type="button" onClick={copyShape}>{copied ? (locale === 'de' ? 'Kopiert' : locale === 'es' ? 'Copiado' : locale === 'fr' ? 'Copié' : locale === 'pt-br' ? 'Copiado' : 'Copied') : (locale === 'de' ? 'Mein Politangle kopieren' : locale === 'es' ? 'Copiar mi Politangle' : locale === 'fr' ? 'Copier mon Politangle' : locale === 'pt-br' ? 'Copiar meu Politangle' : 'Copy my Politangle')}</button>
          <span>{locale === 'de' ? 'Diese achtseitige Form ist dein Politangle. Die Einordnung daneben zeigt, welche politischen Traditionen dem Gesamtmuster am nächsten kommen.' : locale === 'es' ? 'Esta figura de ocho lados es tu Politangle. La lectura explica qué tradiciones políticas se parecen más al conjunto del perfil.' : locale === 'fr' ? 'Cette forme à huit côtés est ton Politangle. La lecture explique quelles traditions politiques se rapprochent le plus de l’ensemble du profil.' : locale === 'pt-br' ? 'Esta forma de oito lados é o seu Politangle. A leitura ao lado mostra quais tradições políticas mais se aproximam do conjunto do perfil.' : 'This eight-sided shape is your Politangle. The reading beside it explains the political traditions that most closely resemble the whole pattern.'}</span>
        </div>
      </article>

      <article className="engine-card result-story-card compact-result-card" style={{ marginTop: 18 }}>
        <p className="engine-kicker">{locale === 'de' ? 'Warum dieses Ergebnis?' : locale === 'es' ? '¿Por qué este resultado?' : locale === 'fr' ? 'Pourquoi ce résultat ?' : locale === 'pt-br' ? 'Por que este é o seu resultado?' : 'Why this is your result'}</p>
        <h2>{locale === 'de' ? 'Die drei Positionen, die dein Profil am stärksten prägen.' : locale === 'es' ? 'Las tres posiciones que más marcan tu perfil.' : locale === 'fr' ? 'Les trois positions qui structurent le plus ton profil.' : locale === 'pt-br' ? 'As três posições que mais moldam seu perfil.' : 'The three positions that shape your profile most.'}</h2>
        ...
        <div className="consistency-quick">
          ...
          <p><b>{locale === 'de' ? 'Stimmigkeit der Antworten.' : locale === 'es' ? 'Coherencia de las respuestas.' : locale === 'fr' ? 'Cohérence des réponses.' : locale === 'pt-br' ? 'Coerência das respostas.' : 'Response coherence.'}</b> {locale === 'de' ? 'Hier wird geprüft, ob Antworten auf ähnliche Grundsatz- und Praxissituationen in dieselbe Richtung zeigen. Das sagt nichts darüber aus, ob deine politische Haltung richtig, informiert oder besonders stark ist.' : locale === 'es' ? 'Esto comprueba si tus respuestas a principios y situaciones prácticas relacionadas apuntan en direcciones parecidas. No juzga si tus ideas políticas son correctas, informadas o firmes.' : locale === 'fr' ? 'Cela vérifie si tes réponses à des principes et à des situations concrètes proches vont dans le même sens. Ce score ne juge pas si tes idées politiques sont justes, informées ou fortement ancrées.' : locale === 'pt-br' ? 'Isso verifica se suas respostas a princípios e situações práticas relacionadas apontam em direções parecidas. Não avalia se suas ideias políticas são corretas, bem informadas ou firmes.' : 'This checks whether your answers to related principle and practical-choice questions point in similar directions. It does not judge whether your politics are correct, informed or strongly held.'}</p>
        </div>

        {largestGap && largestGap.gap >= 20 && (() => {
          const display = axisDisplay(locale, largestGap);
          return <div className="result-tension"><strong>{locale === 'de' ? `Der größte Unterschied zwischen Grundsatz und praktischer Entscheidung liegt bei ${display.name}.` : locale === 'es' ? `La mayor diferencia entre principio y decisión práctica aparece en ${display.name}.` : locale === 'fr' ? `Le plus grand écart entre principe et choix concret apparaît sur ${display.name}.` : locale === 'pt-br' ? `A maior diferença entre princípio e decisão prática aparece em ${display.name}.` : `Your largest principle-to-choice shift is on ${display.name}.`}</strong><p>{locale === 'de' ? `Dein Grundsatzwert liegt bei ${largestGap.think}, deine praktische Entscheidung bei ${largestGap.act}. Die Differenz von ${largestGap.gap} Punkten zeigt, dass konkrete Zielkonflikte deine Antwort hier spürbar verändern.` : locale === 'es' ? `Tu posición de principio es ${largestGap.think} y tu elección práctica ${largestGap.act}. La diferencia de ${largestGap.gap} puntos indica que los dilemas concretos cambian bastante tu respuesta en este tema.` : locale === 'fr' ? `Ta position de principe est à ${largestGap.think}, ton choix concret à ${largestGap.act}. L’écart de ${largestGap.gap} points montre que les arbitrages concrets changent nettement ta réponse sur ce sujet.` : locale === 'pt-br' ? `Sua posição de princípio está em ${largestGap.think}, enquanto sua decisão prática está em ${largestGap.act}. A diferença de ${largestGap.gap} pontos mostra que dilemas concretos mudam de forma perceptível sua resposta neste tema.` : `Your principle position is ${largestGap.think}, while your practical-choice position is ${largestGap.act}. That ${largestGap.gap}-point difference suggests that concrete trade-offs change how you answer on this topic.`}</p></div>;
        })()}
      </article>

      <details className="engine-card result-details-card" style={{ marginTop: 18 }}>
        <summary>{locale === 'de' ? 'Alle acht politischen Achsen ansehen' : locale === 'es' ? 'Ver los ocho ejes políticos' : locale === 'fr' ? 'Voir les huit axes politiques' : locale === 'pt-br' ? 'Ver os oito eixos políticos' : 'See all eight political axes'}</summary>
