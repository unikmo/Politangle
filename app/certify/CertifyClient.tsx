...
type Status = { authenticated: boolean; emailVerified?: boolean; adultConfirmed?: boolean; readiness: { enabled: boolean; bankReady: boolean; bankVersion: string }; attempts?: { allowed: boolean; remaining: number; nextEligibleAt?: string } };
...
export default function CertifyClient({ renewCertificateId }: { renewCertificateId?: string }) {
  ...
  const canStart = Boolean(status?.authenticated && status.emailVerified && status.adultConfirmed && status.readiness.enabled && status.readiness.bankReady && status.attempts?.allowed);
